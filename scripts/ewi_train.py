"""
Train the conditional-diffusion + GRU model exactly as in
conditional_diffusion_ewi.ipynb (cells 1-11, 19-20) and checkpoint
everything needed for ewi_detect.py to run detection without retraining.
"""
...
class NoiseSchedule:
    def __init__(self, T=200, beta_min=0.1, beta_max=0.9):
        self.T = T
...
N_NULL = 5000
null_S = (np.array([mmd2_vs_gaussian(rng.standard_normal((W_MMD, D))) for _ in range(N_NULL)]) - MU0)
print(f"S~G0 (N={N_NULL}): mean={null_S.mean():.6f} std={null_S.std():.6f}")
...
V1_SR      = V1_EST
print(f"pilot p1: delta2={DELTA2_EST:.5f} v1={V1_EST:.5f} alpha_sr={ALPHA_SR:.3f}")

with open(f'{CKPT_DIR}/calib.pkl', 'wb') as f:
    pickle.dump(dict(
        D=D, W_MMD=W_MMD, SIGMA_MMD=SIGMA_MMD, C_CONST=C_CONST, MU0=MU0,
        null_S=null_S, DELTA2_EST=DELTA2_EST, V1_EST=V1_EST,
        ALPHA_SR=ALPHA_SR, V1_SR=V1_SR,
        HIDDEN_ENC=HIDDEN_ENC, HIDDEN_DEN=HIDDEN_DEN, N_LAYERS=N_LAYERS,
        T_DIFF=T_DIFF, BETA_MIN=BETA_MIN, BETA_MAX=BETA_MAX, N_DDIM=N_DDIM,
    ), f)
