"""New illustrative 2D density pair for the paper figure (2nd attempt --
checkerboard's hard edges/multimodality turned out too hard for a
3000-step budget, giving a weaker signal than Ring->Heart, the opposite of
what was wanted). This one is smooth and rotationally symmetric so a small
model learns it well quickly, while still being visually and thematically
distinct from the Spirals panel (no winding, purely radial):

  p0: isotropic Gaussian "blob" centered at the origin, std=1.0
      -- mass concentrated AT the center.
  p1: thin ring at radius r0=2.5, radial noise 0.2
      -- mass entirely AWAY from the center, in an annulus.

The transition is a pure "hollowing out": a filled disk grows an empty
core and its mass migrates outward into a ring. Both are rotationally
symmetric (no preferred angle), so the visual is a clean radial-density
story, easy to see and easy for the network to learn.

Architecture: same as ewi/heart -- v-prediction, LayerNorm-residual, 3000
steps.
"""
